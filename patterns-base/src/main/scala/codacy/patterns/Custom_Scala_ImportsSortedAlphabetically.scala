...
class Custom_Scala_ImportsSortedAlphabetically(config:Custom_Scala_ImportsSortedAlphabetically.Configuration=Custom_Scala_ImportsSortedAlphabetically.Configuration())
  ...
  override def apply(tree: Tree): Iterable[Result] = {
    tree.collect{
      ...
      collect{ case (Some(parent),importsWithImporters) =>
        ...
        parent.children.foldLeft(Seq.empty[Seq[Importer]]){ case (lists, elem) =>
          val currentLastList = lists.lastOption.getOrElse(Seq.empty)

          treeToImporter.get(elem).map{ case importer =>
  ...
  private[this] def firstUnsortedInBlock(block:Seq[Importer], caseSensitive:Boolean) = {
    ...
    def stringify(importer:Importer): String = if(caseSensitive) importer.toString() else importer.toString().toLowerCase()

    block.zip(block.drop(1)).dropWhile{ case (i1,i2) =>
