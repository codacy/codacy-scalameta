...
case object Custom_Scala_UnusedPrivateVariables extends Pattern{
  ...
  private[this] def isShadowed(name:Term.Name, definingName:Term.Name, stats:Seq[Tree]):Boolean = {
    def rec(treeOpt:Option[Tree]):Boolean = {
      treeOpt match{
        ...
        case Some(tree) if stats.exists(_ == tree) =>
          false
  ...
  private[this] def valDefsWithStats(tree:Tree):List[(Term.Name,Seq[Tree])] = {
    tree.collect{
      //valDefs
      case t@q"..${mods:Seq[Mod]} val ..${patsnel:Seq[Pat]}: $tpeopt = $expr" if isPrivate(mods) =>
      ...
      //paramVals
      case t@param"..$mods ${paramname:Term.Name}: $atpeopt = $expropt" if isPrivate(mods) =>
        //first parent should be the constructor def, 2nd the class itself
        t.parent.flatMap(_.parent).map{ case classDef => (classDef,List(paramname)) }
    }.flatten
      .groupBy{ case (template,_) => template }.map{ case (template,lists) =>
      (template,lists.map{ case (tree,names) => names }.flatten)
  ...
  private[this] def companionStats(name:Type.Name, tree:Tree):Option[Tree] = {
    tree.collect{ case q"..$mods object ${oName:Term.Name} extends $template" if name.toString() == oName.toString =>
      template
    }.headOption
  }
