...
case object Custom_Scala_NonFatal extends Pattern{
  ...
  override def apply(tree: Tree) = {
    tree.collect{
      case t@q"try $expr catch { ..case $cases } finally $expropt" =>
        cases.collect{ case cs if isNonFatalCatch(cs) =>
  ...
  private[this] lazy val allEvilSymbols = Set(vmErr,thrErr,intErr,linErr,ctrlErr,genErr,genErr2).flatMap{ case tpe =>
