...
object CheckResources{
  ...
  private def allDescriptions(classLoader:URLClassLoader, dest:File) = {
    ...
    classLoader.findResources("docs/description").map(_.toFile).flatMap{ folder =>
  ...
  private def allTests(classLoader:URLClassLoader, dest:File) = {
    ...
    classLoader.findResources("docs/tests").map(_.toFile).flatMap{ folder =>
      folder.list
    }.map{ testFile =>
  ...
  private def allSources(classLoader:URLClassLoader, dest:File) = {
    ...
    classLoader.findResources("patterns").map(_.toFile).flatMap{ folder =>
      folder.list
    }.map{ sourceFile =>
  ...
  private def descriptionsJson(classLoader:URLClassLoader, dest:File) = {
    val allDescriptions = classLoader.findResources("docs/description/description.json").flatMap{ uri =>
