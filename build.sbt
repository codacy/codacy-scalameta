...
lazy val genResources = project
  .in(file("genResources"))
  .settings(scalaVersion := scalaV)
  .settings(
    (unmanagedClasspath in Compile) ++= Option((baseDirectory.value / ".." / "lib").listFiles()).toList
      .flatMap(_.toList)
  )
...
lazy val basePatterns = project
  .in(file("patterns-base"))
  .settings(scalaVersion := scalaV)
  .dependsOn(core)
...
lazy val common = project
  ...
  .dependsOn(core)
  .settings(scalaVersion := scalaV)
  .settings(libraryDependencies ++= Seq(Dependencies.codacyEngine, Dependencies.betterFiles))
...
resourceGenerators in Compile += Def.task {
  ...
  val result = (runner in Compile).value.run(
    "codacy.CheckResources",
    (dependencyClasspath in Compile).value.files ++ (unmanagedClasspath in Compile).value.files,
    Array(file.getPath),
    streams.value.log
  )
...
dockerBaseImage := "amazoncorretto:8u382-alpine3.18-jre"
